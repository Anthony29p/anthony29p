<h1 align="center">Hi, I'm Anthony Perez</h1>
<h3 align="center">Backend Developer & Industrial Engineer from Madrid, Spain</h3>

<p align="center">
<em>Passionate Backend developer specialized in Go and microservices, with experience building scalable and high-performance systems. I have a strong focus on delivering clean and readable code, and I take pride in my ability to solve problems and learn constantly.</em>
</p>

---

## 🚀 Current Role
**Backend Developer (Go)** *(Present)*
- Building microservices systems from early stages using Go and hexagonal architecture
- Integrating multiple external systems and data sources with event-driven pipelines
- Managing cloud infrastructure as code and deployments on Kubernetes (GKE)

## 💻 Technical Skills

### **Languages:**
<div>
<img align="center" title="Go" alt="golang" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" />
<img align="center" title="Typescript" alt="typescript" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" />
<img align="center" title="JavaScript" alt="Js" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-plain.svg">
<img align="center" title="Solidity" alt="Solidity" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/1119b9f84c0290e0f0b38982099a2bd027a48bf1/icons/solidity/solidity-original.svg">
</div>

### **Backend & Architecture:**
<div>
<img align="center" title="Go" alt="golang" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg" />
<img align="center" title="Node.js" alt="nodejs" height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/nodejs/nodejs.png">
<img align="center" title="NestJS" alt="nestjs" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nestjs/nestjs-original.svg">
</div>

**Go:** Gin, Fiber, GORM | **Patterns:** Microservices, Hexagonal Architecture, Event-Driven, TDD | **Node.js:** Express, NestJS, Koa, Inversify

### **Cloud & Infrastructure:**
- **GCP:** GKE, Pub/Sub, Cloud Scheduler, BigQuery, Cloud SQL, Firestore, Cloud Storage, Secret Manager
- **AWS:** Lambda, API Gateway, SQS, EventBridge, RDS, DynamoDB, ElastiCache, EKS, Cognito
- **IaC & DevOps:** Terraform, Terragrunt, Kubernetes, Kustomize, Skaffold, Docker, GitLab CI/CD, Cloud Build
- **Observability:** OpenTelemetry, CloudWatch

### **Databases:**
<div>
<img align="center" title="PostgreSQL" alt="postgres" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/1119b9f84c0290e0f0b38982099a2bd027a48bf1/icons/postgresql/postgresql-plain.svg">
<img align="center" title="MongoDB" alt="mongo" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg">
<img align="center" title="MySQL" alt="mysql" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg">
<img align="center" title="Redis" alt="redis" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg">
</div>

### **Frontend:**
<div>
<img align="center" title="React" alt="React" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg">
<img align="center" title="Next.js" alt="nextjs" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg">
</div>

## 🌍 Languages
- 🇪🇸 **Spanish:** Native
- 🇧🇷 **Portuguese:** C1
- 🇺🇸 **English:** B2
- 🇫🇷 **French:** B1
- 🇮🇹 **Italian:** B1
- 🇩🇪 **German:** A2

## 🎓 Education
**Industrial Engineering** - National University of Engineering (2011-2016)

## 📊 GitHub Stats
<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=anthony29p&show_icons=true&theme=gotham" alt="GitHub Stats" />
</p>

## 📫 Contact Me
<div align="center">
<a href="https://anthonyperez.dev/">
<img alt="Website" width="22px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/chrome/chrome-original.svg" />
</a>
&nbsp;&nbsp;
<a href="https://www.linkedin.com/in/anthony-p-53962163/">
<img alt="LinkedIn" width="22px" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" />
</a>
&nbsp;&nbsp;
<a href="mailto:anthonyperezalbornoz@gmail.com">
<img alt="Email" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/gmail.svg" />
</a>
</div>
